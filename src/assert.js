...
const arrlen = (exports.arrlen = (val, name, len) => (units) => {
  assert(type(val)(units) == "array", `Field "${name}" should be an array!`);
  let valid = false;
  let lenstr = "";
  if (type(len)(units) == "array") {
    valid = len.some((l) => val.length == l);
    lenstr = len.join(" or ");
  } else {
    valid = val.length === len;
    lenstr = `${len}`;
  }
  assert(valid, `Field "${name}" should be an array of ${lenstr} elements`);
  return val;
});
...
const numarr = (exports.numarr = (raw, name, length) => (units) =>
  arr(raw, name, length, "number", 0)(units));
const strarr = (exports.strarr = (raw, name) =>
  arr(raw, name, 0, "string", "")());
